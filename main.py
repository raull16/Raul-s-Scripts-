...
class RedeemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with db.acquire() as conn:
            ...
            key_data = await conn.fetchrow("SELECT * FROM keys WHERE key = $1 AND used = 0", key)
            if key_data:
                await conn.execute("UPDATE keys SET used = 1, used_by = $1 WHERE key = $2", interaction.user.id, key)
...
async def setpanel(interaction: discord.Interaction, loader: str):
    ...
    async with db.acquire() as conn:
        await conn.execute(
            ...
            interaction.guild_id, interaction.channel_id, f'{{"loader": "{loader}", "created_by": {interaction.user.id}}}'
        )
